...
def train_model(model, data, seed=None, output_noise_scale=0.0):
    ...
    for i in tqdm.trange(1, len(data)):
        out_noise = output_noise_scale * rng.uniform(-1, 1, outputs[i-1].shape) # added xavier
        z = (np.dot(model["W_rc"], internals[i-1]) +
             np.dot(model["W_in"], inputs[i]) +
             model["scaling"] * np.dot(model["W_fb"], outputs[i-1] + out_noise)) #TODO: il faudrait enlever le scaling a chaque fois
...
def test_model(model, data, seed=None, output_noise_scale=0.0):
    ...
    for i in tqdm.trange(1,len(data)+1):
        ...
        z = ( np.dot(model["W_rc"], internals[i-1]) +
              np.dot(model["W_in"], inputs[i]) +
              model["scaling"]*np.dot(model["W_fb"], outputs[i-1])) #TODO: il faudrait enlever le scaling a chaque fois
        ...
        outputs[i] = np.dot(model["W_out"], internals[i]) + out_noise # add xav
